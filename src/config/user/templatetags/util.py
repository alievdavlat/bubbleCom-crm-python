...
def total_product_price(qty, price):
    ...
    price = int(qty) * (int(price) * int(currency.amount))
...
def total_price(order_item, type):
    ...
    for item in order_item:
        if type == 'A':
            total += int(item.product.price['A']) * int(item.amount)
        elif type == 'B':
            total += int(item.product.price['B']) * int(item.amount)
        elif type == 'C':
            total += int(item.product.price['C']) * int(item.amount)
    return price_format(total * int(currency.amount) )
...
def discount_price(order_item, type, discount):
    ...
    discount_price = (total * int(currency.amount)) * (discount / 100)
...
def total(price, discount, delivery_price):
    total = price
    # discount_price = total * (discount / 100) if discount else 0
    # total = (total - discount_price) + delivery_price
