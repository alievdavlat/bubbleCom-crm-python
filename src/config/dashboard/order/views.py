    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for i in range(count):
                ...
                if product:
                    product.amount -= int(request.POST.getlist('product_option_value')[i])
                    product.save()
                if order_item:
                    order_item.amount += int(request.POST.getlist('product_option_value')[i])
                    order_item.save()
                    amount_price += int(order_item.product.price[model.price_type]) * int(order_item.amount)
            ...
            if model.discount:
                discount_price = (amount_price * int(currency_price.amount)) * (model.discount / 100)
                model.price = (amount_price * int(currency_price.amount) )- discount_price
            else :
                model.price = amount_price * int(currency_price.amount)
            if model.qqs:
                qss_price = (amount_price * int(currency_price.amount)) * (12 / 100)
                model.qqs_price = qss_price
                model.price = (amount_price * int(currency_price.amount) ) + qss_price
            model.save()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if model.discount:
                ...
                model.price = amount_price * int(currency_price.amount)
            if model.qqs:
                qss_price = (amount_price * int(currency_price.amount)) * (model.qqs / 100)
                model.qqs_price = qss_price
                model.price = (amount_price * int(currency_price.amount) ) + qss_price
...
@staff_member_required
@role_required(['seo', 'agent', 'driver'])
    ...
    if is_ajax(request=request):
        ...
        if total_price >= model.price + model.delivery_price:
            ...
            for order_item in order_items:
                agent_salary += int(order_item.product.price[model.price_type]) * (int(order_item.product.price[f'percentage_{model.price_type}']) / 100) * int(order_item.amount)    
