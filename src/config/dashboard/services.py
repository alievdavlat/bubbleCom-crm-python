...
def get_debtors():
    sql_query = """
       SELECT c.id AS client_id, json_build_object(
							...
							o.id AS order_id,  o.price - COALESCE(SUM(opc.price), 0) AS remaining_balance
...
def _format_products_income(item):
    ...
    return OrderedDict([
        ...
        ("production_product_price",
         int(json.loads(item['production_product_price'])['A']) * int(item['production_product_amount']) if item['production_product_price'] else 0),
